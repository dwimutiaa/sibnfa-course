...
import "../styles/GenreAdmin.css"; // <-- panggil css
...
export default function GenreAdmin() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(err);
      setError("Kamu gagal menghapus Genre");
    }
  };
    ...
    <>
      ...
      <div className="page-container">
        ...
        <form className="simple-form" onSubmit={handleCreate}>
          <input type="text" name="name" placeholder="Nama Genre" value={form.name} onChange={handleChange} required />
          <input type="text" name="description" placeholder="Deskripsi (opsional)" value={form.description} onChange={handleChange} />
          ...
          <tbody>
            {genres.map((g) => (
              <tr key={g.id}>
                ...
                <td>{g.description || "-"}</td>
                <td>
                  <button className="btn-danger" onClick={() => handleDelete(g.id)}>Hapus</button>
                </td>
              </tr>
            ))}
          </tbody>
